    ...
    foreach ($line in $reflist) {
        ...
        write-host $line
            if ($line.Contains(").")) {
                ...
                $titlestart = $line.indexof(").")+1
                $titleend = $line.indexof(".",$titlestart+2)
                print($titlestart)
                print($titleend)
                Write-Host "Extracted title: $($line.Substring($titlestart,($titleend-$titlestart)).Trim())"
                $Title+= $line.Substring($titlestart,($titleend-$titlestart)).Trim()
    ...
    $reflist=$reflist.TrimEnd("`r","`n")
        ...
        for ($j=0; $j -lt $reflist.Count; $j++) {
           ...
           write-host "Authors: " + $authors[$j] + "`r`n"
              ...
              Write-Host "Title: " + $Title[$j] + "`r`n"
              write-host "DOI Title: " + $DOITitle[$j] + "`r`n"
              write-host "Valid DOI URL: " + $doiUrl[$j] + "`r`n"
              print($Title[$j])
        }
        Showresultsform -authors $authors -doiUrl $doiUrl -validURL $validURL -DOIAuthors $DOIAuthors -DOITitle $DOITitle -Title $Title -reflist $reflist
    ...
    function Showresultsform {
        ...
        for ($i=0; $i -lt $reflist.Count; $i++) {
            ...
            $backColorcount++
                ...
                $textBox.SelectionColor = $backcolor
                $textBox.AppendText("Title: " + $Title[$i] + "`r`n")
                $textBox.SelectionColor = $backcolor
...
$formWidth= $($form.Width)-40
$formHeight= $($form.Height)-350

$textBox.Size = New-Object System.Drawing.Size($formWidth, $formHeight)
$textBox.Multiline = $true
